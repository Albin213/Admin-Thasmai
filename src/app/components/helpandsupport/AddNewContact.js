    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/superadmin/listDepartments`
        );
    ...
    if (departments && name && contact) {
      try {
        const response = await axios.post(
          `${process.env.NEXT_PUBLIC_API_URL}/superadmin/addContact`,
          newContactDetails
        );
        ...
        <div className="w-full h-[344px] px-10 py-8 flex flex-col justify-between">
          ...
          >
            ...
            {departmentList.map((dept, index) => {
              return (
                <option key={dept.id} value={dept.departments}>
                  {dept.departments}
                </option>
              );
            })}
          ...
          <input
            ...
            onChange={(e) => {
              ...
              // Allow input only if the length is less than or equal to 10
              if (value.length <= 10) {
                handleOnChange(e); // Proceed with handling the change
              }
            }}
          />
